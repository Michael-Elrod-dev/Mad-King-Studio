  ...
  if (error) {
    return (
      <div className="bg-red-900/20 border border-red-700 rounded-lg p-6 text-center">
        <h3 className="text-red-400 font-semibold mb-2">Error Loading Dev Logs</h3>
    ...
    return (
      <div className="bg-neutral-800 rounded-lg p-8 text-center">
        ...
        <p className="text-neutral-50 mb-4">
          Development logs will appear here as they&apos;re added to the GitHub repository.
        </p>
        <a
        ...
        <div className="text-center pt-8">
          ...
          >
            {isLoadingMore ? (
              ...
              `Load More Posts (${Math.min(POSTS_PER_PAGE, devLogs.length - visiblePosts)} more)`
            )}
          </button>
        </div>
      )}
      ...
      {!hasMorePosts && devLogs.length > POSTS_PER_PAGE && (
        <div className="text-center pt-8">
          <p className="text-neutral-400 text-sm">
            You&apos;ve reached the end. All {devLogs.length} development logs are now visible.
          </p>
        </div>
      )}
